...
import { RequestValidationError, RevisionMismatchError, type VenueRuntime } from "./venueRuntime.ts";
...
async function route(runtime: VenueRuntime, request: IncomingMessage, response: ServerResponse): Promise<void> {
  const url = new URL(request.url ?? "/", "http://venue-runtime.local");
  ...
  if (url.pathname === "/api/select" && request.method === "POST") {
    json(response, await runtime.select(await readJson(request) as Parameters<VenueRuntime["select"]>[0]));
    return;
  }
  if (url.pathname === "/api/control" && request.method === "POST") {
    const body = await readJson(request);
    json(response, runtime.control(body.action));
    return;
  }
  ...
  if ([
    "/api/health", "/api/status", "/api/display", "/api/display/events", "/api/select", "/api/control",
