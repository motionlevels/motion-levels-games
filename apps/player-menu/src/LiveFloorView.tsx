...
export function liveFloorEventsURL(): string {
  return `${engineBaseURL().replace(/\/+$/u, "")}/api/live-floor/events`;
}
...
export function LiveFloorView({ orientation = "landscape" }: { orientation?: LiveFloorOrientation }) {
  ...
  useEffect(() => {
    ...
    function applyFrame(buffer: ArrayBufferLike) {
    ...
    function connect() {
      ...
      lastFrameAtRef.current = Date.now();
      const stream = new EventSource(liveFloorEventsURL());
      streamRef.current = stream;
      stream.addEventListener("open", () => setConnection("connecting"));
      stream.addEventListener("live-floor", (event) => {
        try {
          const frame = JSON.parse(event.data) as ObservedFloorFrame;
          if (!frame.frameBase64) return;
          const binary = window.atob(frame.frameBase64);
          const bytes = new Uint8Array(binary.length);
          for (let index = 0; index < binary.length; index += 1) bytes[index] = binary.charCodeAt(index);
          applyFrame(bytes.buffer);
        } catch {
          // A malformed observed frame is ignored; the freshness watchdog will reconnect.
        }
      });
    ...
    const watchdog = window.setInterval(() => {
      ...
      if (!stream || stream.readyState !== EventSource.OPEN || Date.now() - lastFrameAtRef.current <= 4_000) return;
      setConnection("error");
      stream.close();
