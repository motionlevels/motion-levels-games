    ...
    stdin: {
      contents: `
        ...
        window.displayLoaderHarness = {
          states,
          render(revision, score) {
            root.render(createElement(MotionLevelsGamesDisplay, {
              fallback: createElement("span", null, "fallback"),
              onStateChange,
              status: {
                currentGame: "motion-levels-games:fixture-game",
                frame: undefined,
                gameSnapshot: { currentGame: "fixture-game", phase: "running", score },
                phase: "running",
...
async function bundleDisplayRuntime(revision: string, styles: string): Promise<string> {
  ...
  const result = await build({
    ...
    entryPoints: ["packages/runtime/src/display.tsx"],
    format: "iife",
  ...
  return `
    (() => {
      ...
      const record = (kind, input) => {
        ...
        window.fixtureDisplayEvents.push({ kind, revision, score: input?.snapshot?.score });
      };
      const paint = (host, kind, input) => {
        record(kind, input);
        const marker = document.createElement("span");
        marker.className = "historical-style-marker";
        marker.textContent = kind + ":" + revision + ":" + input.snapshot.score;
...
declare global {
  interface Window {
    ...
    displayLoaderHarness: {
      render(revision: string, score: number): void;
      states: Array<{ status: string }>;
    };
    fixtureDisplayEvents: Array<{ kind: string; revision: string; score?: number }>;
...
type FixtureRuntime = {
  revision: string;
  mount(host: Element, input: { snapshot: { score?: number } }): void;
  update(host: Element, input: { snapshot: { score?: number } }): void;
