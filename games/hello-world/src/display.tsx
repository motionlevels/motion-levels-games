...
export function PlayerDisplay({
  ...
  const statusValue = finished ? (
    <span className="hello-world-result-copy">
      <span>{snapshot.success ? "¡Ganaste!" : snapshot.lastEventMessage}</span>
      <small>Reinicio en {restartSeconds}</small>
    </span>
  ) : snapshot.lastEventMessage || "Verde suma, rojo resta una vida";

  return (
    <GameDisplayShell title={snapshot.label} phase={snapshot.phase}>
      <div className={`ml-solo-display hello-world-display ${resultClass}`.trim()}>
