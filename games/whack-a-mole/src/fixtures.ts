...
function create(stage: "waiting" | "starting" | "running" | "finished") {
  ...
  if (stage === "running" || stage === "finished") game.tick({ atMillis: 2_200 });
  if (stage === "finished") game.tick({ atMillis: 5_300 });
  return game;
}
...
const running = create("running"); const target = running.snapshot().targets[1]!; running.press({ x: target.x, y: target.y, pressed: true, atMillis: 2_300 }); export const runningFrame = running.render(); export const runningSnapshot = running.snapshot();
