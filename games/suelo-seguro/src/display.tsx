    ...
    <GameDisplayShell title={snapshot.label} phase={shellPhase}>
      <div className="suelo-seguro-display" style={style}>
          ...
          <section className="suelo-seguro-players" aria-label="Jugadores">
            {snapshot.players.map((player) => (
              <article className={`suelo-seguro-player${player.index === snapshot.activePlayerIndex ? " is-active" : ""}`} key={player.index} style={{ "--player-color": player.color } as CSSProperties}>
                <i /><span>{player.label}</span><strong>{player.score}</strong>
              </article>
            ))}
          </section>
        </main>
        <aside className="suelo-seguro-sidebar">
          ...
          <article className="suelo-seguro-card"><span>Relevos seguros</span><strong>{snapshot.completedTransfers}/{snapshot.requiredTransfers}</strong><div className="suelo-seguro-progress"><i /></div></article>
          <article className="suelo-seguro-card"><span>Tiempo de partida</span><strong>{formatClock(snapshot.remainingMillis)}</strong></article>
          <div className="suelo-seguro-event">{snapshot.lastEventMessage || "El suelo está preparado"}</div>
        </aside>
        {snapshot.phase === "round-win" ? <Result className="is-round" title={`¡${snapshot.activePlayerLabel} está a salvo!`} caption={`${snapshot.completedTransfers} de ${snapshot.requiredTransfers} relevos completados`} /> : null}
        {snapshot.phase === "turn-fail" ? <Result className="is-fail-turn" title="Turno perdido" caption="La siguiente plataforma ya está activa" /> : null}
        {snapshot.phase === "finished" ? <Result className={snapshot.success ? "is-game-win" : "is-game-fail"} title={snapshot.success ? "¡Equipo a salvo!" : "El rojo os alcanzó"} caption={snapshot.success ? `${snapshot.completedTransfers} relevos sin rendirse` : "Busca tu color y esquiva el patrón"} /> : null}
      </div>
    </GameDisplayShell>
  );
}
