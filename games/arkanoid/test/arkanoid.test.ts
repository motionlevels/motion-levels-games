...
test("ready floor shows the complete brick wall, paddle, and ball", () => {
  ...
  assert.equal(events[0]?.message, "Pisa abajo para lanzar");
...
test("only the lower control zone moves and launches the paddle", () => {
  const game = createGame({ playerCount: 1 });
  game.init(0);
...
test("difficulty changes ball speed through deterministic movement", () => {
  ...
  easy.tick({ atMillis: 1300 });
  expert.tick({ atMillis: 1300 });
...
function advanceWithTracking(game: ArkanoidGameInstance, durationMillis: number): void {
  for (let nowMillis = 150; nowMillis <= durationMillis; nowMillis += 50) {
    const snapshot = game.snapshot();
