...
```sh
npm run create:game -- memory-challenge "Reto de memoria"
```

## Gameplay

One to four players each receive a separate floor lane. Everyone occupies the
illuminated exit, watches a deterministic colored path, and then races across
the same path after it fades into animated lava. A wrong tile resets that
player's progress; stepping back onto their exit reveals the route for another
attempt. The first complete path wins, or the race ends after 90 seconds.

## Development

```sh
npm run test --workspace @motion-levels-games/memory-challenge
npm run typecheck --workspace @motion-levels-games/memory-challenge
```
...
Player count is strict because it changes lane width, readiness zones, paths,
and the race roster. All supported counts from one through four are tested.
